...
def fac_said_gaussian_ramp_2d(E: xarray.Dataset, gridflag: int, flagdip: bool) -> xarray.Dataset:
    """
    ...
    auxlengthcenter=np.floor(aux.shape[0]/6)
    ...
    for t in range(1,auxlength+1):
        ...
        if t>(auxlengthcenter):
            E[k].loc[E.time[t]] = E.Jtarg * shapelat
        else: 
            E[k].loc[E.time[t]] = E.Jtarg * shapelat * (1/(auxlengthcenter) * (t-1))
